...
def train(train_df, valid_df, test_df):
    ...
    label_size = len(set(train_df.target.unique().tolist() + valid_df.target.unique().tolist()))
    ...
    tfv.fit(list(train_df.text.values) + list(valid_df.text.values))
    ...
    for fold_idx, (train_idx, valid_idx) in enumerate(kf.split(X=train_df["text"],
        ...
        valid = tfv.transform(valid_df["text"].values)
        X_test = tfv.transform(test_df["text"].values)
